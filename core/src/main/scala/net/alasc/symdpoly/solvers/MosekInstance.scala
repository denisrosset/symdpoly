...
class MosekInstance(val relaxation: Relaxation[_, _, _]) extends Instance {
...
class MosekInstance2(val relaxation: Relaxation2[_, _]) extends Instance2 {
  ...
  // LMI constant
  val c = SparseMatrix.forMoment2(gramMatrix, 0)

  // LMI matrices
  val a = Vector.tabulate(m)(i => SparseMatrix.forMoment2(gramMatrix, i + 1, -1.0))
