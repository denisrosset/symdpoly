...
class PairRules[F <: free.MonoidDef.Aux[F] with Singleton](val width: Int, val lookup: Array[Int], val custom: Map[(F#Op, F#Op), Mono[F, F]])(implicit wF: Witness.Aux[F]) {
  ...
  def applyRules(op1: F#Op, op2: F#Op): Mono[F, F] = rule(F.indexFromOp(op1), F.indexFromOp(op2)) match {
    case PairRules.SetToZero => symdpoly.Mono.zero[F, F]
    case PairRules.Preserve => symdpoly.Mono(op1, op2)
    case PairRules.RemoveBoth => symdpoly.Mono.one[F, F]
    case PairRules.Swap => symdpoly.Mono(op2, op1)
    case PairRules.KeepFirst => symdpoly.Mono(op1)
