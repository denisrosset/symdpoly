...
package object quotient {
  ...
  type Rules[F <: free.MonoDef.Aux[F] with Singleton] = Seq[(Mono[F, F], Mono[F, F])]

  def pairs[F <: free.MonoDef.Aux[F] with Singleton:Witness.Aux](rules: PairSubstitutions[F]): Rules[F] = for {
  ...
  def commutative[F <: free.MonoDef.Aux[F] with Singleton:Witness.Aux]: Rules[F] = for {
  ...
  def rules[F <: free.MonoDef.Aux[F] with Singleton](pairs: (Mono[F, F], Mono[F, F])*): Rules[F] = pairs
