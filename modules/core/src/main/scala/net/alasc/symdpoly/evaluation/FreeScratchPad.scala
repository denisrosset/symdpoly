...
/** Scratch pad of mutable words where equivalence operations are applied in batch.
  * The pad itself is an array of words, all stored in their phase canonical form. Additionally, the map "phaseMap"
  ...
  *
  * - They are locked.
  * - The canonical phase representatives are stored in the "pad" array, while the corresponding phase is stored .
  *   in "phaseArray", so that the i-th stored monomial is pad(i).mutableCopy.setPhaseTo(Phase.fromEncoding(phaseArray(i))).
  * - For fast lookup and test of equivalence to zero, we have phaseMap(pad(i)) = phaseArray(i).
  *
  * For the words at index >= n:
  *
  * - They are mutable.
  * - They are not in their phase canonical form.
  * - The corresponding entries in phaseArray are ignored.
  *
